...
const submit = async () => {
  try {
    ...
    if (payload.value.id) {
      const res = await axios.put('estates/' + payload.value.id, payload.value)
      if (res.request.status === 200) {
        onDone.value = true
      }
    } else {
      const res = await axios.post('estates', payload.value)
      if (res.request.status === 200) {
        onDone.value = true
      }
    }
...
const openEditDialog = record => {
  showDialog.value = true
  steperTitle.value = 'ویرایش دارایی'
  payload.value.id = record.id
  payload.value.estate_owner = record.estate_owner
  payload.value.estate_type = record.estate_type
  payload.value.price = record.price
