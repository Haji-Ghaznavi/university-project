...
import ProductionCapacitySteper from '@/components/ProductionCapacitySteper/ProductionCapacitySteper.vue'
import usePageConfig from '@/page-configs/production_capacity'
...
const fetchRecord = async (searchValue = null, searchBy) => {
  try {
    loading.value = true
    const { data } = await axios.get('production-capacities', {
      params: {
        search: searchValue,
        searchBy: searchBy,
...
const onConfirm = async () => {
  try {
    ...
    const res = await axios.delete('production-capacities/' + selectedRecord.value?.id)
