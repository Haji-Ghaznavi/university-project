...
import CompanyReportSteper from '@/components/CompanyReportSteper/CompanyReportSteper.vue'
import usePageConfig from '@/page-configs/company_reports'
...
const fetchRecord = async (searchValue = null, searchBy) => {
  try {
    loading.value = true
    const { data } = await axios.get('company-reports', {
      params: {
        search: searchValue,
        searchBy: searchBy,
...
const onConfirm = async () => {
  try {
    ...
    const res = await axios.delete('company-reports/' + selectedRecord.value?.id)
