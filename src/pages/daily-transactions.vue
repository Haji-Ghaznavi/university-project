...
import DailyTransactionSteper from '@/components/DailyTransactionSteper/DailyTransactionSteper.vue'
import usePageConfig from '@/page-configs/daily_transaction'
...
const fetchRecord = async (searchValue = null, searchBy) => {
  try {
    loading.value = true
    const { data } = await axios.get('daily-transactions', {
      params: {
        search: searchValue,
        searchBy: searchBy,
...
const onConfirm = async () => {
  try {
    ...
    const res = await axios.delete('daily-transactions/' + selectedRecord.value?.id)
